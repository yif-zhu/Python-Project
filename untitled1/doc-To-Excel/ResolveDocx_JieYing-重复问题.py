...
DATANOTFOUND = 0
writeLog = 0
def arearMapSupInfoToExcelName(table, sBeginRow, sEndRow, sCols, dSheet, dBeginRow, dCols, reportType, dataSource, tableCode, dNode):
    sheet = cwb[dSheet]
    sList = sCols.split(',')
    dList = dCols.split(',')
    dcolsNames = dNode.attrib('colsNames')
    ...
    while sBeginRow <= sEndRow:
        for sCellI in range(len(sList)):
            ...
            if sCol == 'space':
                ...
                scolsValue = dNode.attrib('colsValue').split(',')
                dRow = dBeginRow + dRowIncress
                sheet[dList[0] + str(dRow)] = dNameList[sCellI]
                sheet[dList[1] + str(dRow)] = itemName
...
def main():
    ...
    sourceFilePath = r'C:\\PyCharm\\docx\\weiying222.docx'  # sys.argv[1]
    destFileName = 'LoanServiceReport_6_supInfo4.xlsx'  # sys.argv[3]
    configFilePath = 'C:\\PyCharm\\xml\\weiying2017-2.xml'  # sys.argv[2]
