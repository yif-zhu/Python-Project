...
def cleanOldData():
    sql = "exec DVImport.CleanMissingPeriodLog "
    execSQLCmd(sql)


def runDBDataValidation():
    sql = "exec DV.UpdateMissingPeriodLog "
    execSQLCmd(sql)
...
def specificColsExtract(cfgItem, sheet):
    ...
    while rStart <= rEnd:  # rows loop
        ...
        if (isFirstRow == 1 and firstRowAllNA == 1):
            ...
            isFirstRow = 0
            if rvalues != "":
                tmpl += "(N'{0}'),".format(rvalues.rstrip(','))
            rStart += 1
...
configFilePath = os.path.join(scriptFolder, 'InsertAllocatePeriodLog_setting.xml')
