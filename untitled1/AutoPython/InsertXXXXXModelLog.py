...
def cleanOldData():
    sql = "exec DVImport.CleanXXXXXResultModelLog "
    execSQLCmd(sql)


def runDBDataValidation():
    sql = "exec DV.UpdateXXXXXResultModelLog "
    execSQLCmd(sql)
...
configFilePath = os.path.join(scriptFolder, 'InsertXXXXXModelLog_setting.xml')
