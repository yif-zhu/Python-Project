...
def CheckData(sourceFilePath):
    ...
    try:
        ...
        def Verification(file):
            ...
            for i in range(file['AmountPercentage'].index.values.size):
                ...
                if flag is False:
                    ...
                    ErrorMsg.append("{0}:0:列数值设置有误-AmountPercentage值：{1}不是数字，请修改".format(cell.rstrip(';'), file['AmountPercentage'][val]))
            ...
            if ErrorNum == 0:   #类型全是数字，可以进行加总运算
                ...
                if (AmountPercentage_c > 100.5 or AmountPercentage_c < 99.5) and CountPercentage_c!=0 and AmountPercentage_c!=0:
                    ...
                    ErrorMsg.append("{0}:0:列数值设置有误-AmountPercentage分布总计值：{1}不等于100(忽略精度)".format(cells.rstrip(';'), AmountPercentage_c))
                ...
                if (CountPercentage_c>=100.5 or CountPercentage_c<=99.5) and CountPercentage_c!=0 and AmountPercentage_c!=0:
                    ...
                    ErrorMsg.append("{0}:0:列数值设置有误-CountPercentage分布总计值：{1}不等于100(忽略精度)".format(cells.rstrip(';'), CountPercentage_c))
        ...
        ErrorMsg.append(str(traceback.format_exc()))
