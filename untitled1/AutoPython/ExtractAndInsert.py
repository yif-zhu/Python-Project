...
configFilePath = os.path.join(scriptFolder, 'ExtractAndInsert_Trustee.xml')
...
def getTrustID(trustCode):
    sql = "select TrustId from DV.view_Products where TrustCode = N'{0}' and AssetType = N'CarLoan'".format(trustCode)
    try:
