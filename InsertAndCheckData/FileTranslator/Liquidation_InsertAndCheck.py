...
def cleanOldData():
    sql = "exec DVImport.ClearLastTimeImport_Liquidation {0}, {1}".format(trustID, paymentPeriodID)
    execSQLCmd(sql)


def runDBDataValidation():
    sql = "exec DVImport.CheckDataValidation_Liquidation {0}, {1}".format(trustID, paymentPeriodID)
...
def specificColsExtract(cfgItem, sheet):
    ...
    while rStart <= rEnd:  # rows loop
        ...
        for cell in cNode:  # cells loop
            ...
            if isFirstRow == 1 and (crv == '' or crv == 'NA') and cemptybreak == '1':
                rvalues = ''
                firstRowAllNA = 1
                break  # break cells loop
...
def main(configFilePath, dateId):
    ...
    for dirPath, dirNames, fileNames in os.walk(sourceFolder):
        for fileName in fileNames:
            ...
            paymentPeriodID = 999
            if trustID == '' or trustID == 0 :
                writeErr("\n【{0}】".format(sourceFilePath))
                writeLog("【错误】系统中未能定位到产品，文件名称中TrustCode设置有误")
                writeErr("【错误】系统中未能定位到产品，文件名称中TrustCode设置有误")
                continue
