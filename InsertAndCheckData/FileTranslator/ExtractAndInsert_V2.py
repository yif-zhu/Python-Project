...
def getTrustID(trustCode, assetType):
    sql = "select TrustId from DV.view_Products where TrustCode = N'{0}' and AssetType = N'{1}' and IsMarketProduct = 1".format(trustCode, assetType)
    try:
...
def cleanOldData():
    sql = "exec DVImport.ClearLastTimeImport {0}, {1}".format(trustID, paymentPeriodID)
    execSQLCmd(sql)

def runDBDataValidation():
    sql = "exec DVImport.CheckDataValidation {0}, {1}".format(trustID, paymentPeriodID)
...
def PoolDistributionsImport(Excelfilepath, TrustId, paymentPeriodID):
    ...
    for Cindex in DataPoolDistributionsImport.index:
        ...
        Amount = 'null' if math.isnan(DataPoolDistributionsImport.loc[Cindex][4]) else DataPoolDistributionsImport.loc[Cindex][4]
        AmountPercentage = 'null' if math.isnan(DataPoolDistributionsImport.loc[Cindex][5])  else DataPoolDistributionsImport.loc[Cindex][5]
        Count = 'null' if math.isnan(DataPoolDistributionsImport.loc[Cindex][6]) else DataPoolDistributionsImport.loc[Cindex][6]
        CountPercentage = 'null' if math.isnan(DataPoolDistributionsImport.loc[Cindex][7]) else DataPoolDistributionsImport.loc[Cindex][7]
...
def main(configFilePath, dateId):
    ...
    for dirPath, dirNames, fileNames in os.walk(sourceFolder):
        for fileName in fileNames:
            ...
            cleanOldData()
            extractExcel(excelwb, cfgRoot)  #sheet1内容上传
