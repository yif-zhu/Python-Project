...
explainStr["15"] ="首先请注意模板中绿框填写要求，填0还是填NA；一般认为新增违约贷款金额即指拖欠达90天的贷款金额，一般pdf任意会披露一个数据，或者2个都披露，请按照实际情况填写，是0就填0，为“-”或者为空就填写NA。特殊情况请联系组长"
...
def getValidationResult(ds):
    ...
    NewDefaultLoan = getNumberVal(ds[8][0].val)
    OutThirtyLoan = getNumberVal(ds[8][1].val)

    if NewDefaultLoan is not None and OutThirtyLoan is not None and NewDefaultLoan != OutThirtyLoan:
        ckResult.append("{0};{1}:{2}:【新增违约情况】新增违约贷款金额：{3}与拖欠达90贷款金额：{4}不相等:15".format(ds[8][0].cell
                                                                                   , ds[8][1].cell, 0, NewDefaultLoan, OutThirtyLoan))
