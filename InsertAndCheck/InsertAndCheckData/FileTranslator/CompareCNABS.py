...
def runDBDataValidation(type, poolType, Importime, assetType, TrustId):
    sql = "exec DV.CompareCnabsAndSystem {0},{1},N'{2}',N'{3}','{4}'".format(type, poolType, Importime, assetType, TrustId)
...
def main(configFilePath, dateId):
    ...
    errtxtFilePath = os.path.join(destFolder, 'Error_对比结果错误信息_{0}.xlsx'.format(dateId))
    ...
    if len(dbCheckResult) > 0:
        ...
        writeLog("数据对比完成，错误详情请查看:{0}".format(errtxtFilePath))
        ...
        logSheet["H{0}".format(1)] = dbCheckResult[0].cursor_description[7][0]
        i = 2
        for r in dbCheckResult:
            ...
            logSheet["H{0}".format(i)] = r[7]
            i += 1
        excelwb.save(errtxtFilePath)
    else:
        writeLog("数据对比已完成，无错误！！！")

if __name__ == "__main__":
    filePath = "C:/PyCharm/InsertAndCheckData/FileTranslator/CompareCNABS_Trustee.xml"
    dateId = 2313121
