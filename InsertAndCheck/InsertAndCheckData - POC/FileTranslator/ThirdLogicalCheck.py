...
def runDBDataValidation(type, poolType, ExcelTypeId, Importime, assetType, TrustId):

    sql = "exec DV.InsertToDvAndVerifyData {0},{1},N'{2}',N'{3}',N'{4}',N'{5}'".format(type, poolType, ExcelTypeId, Importime, assetType, TrustId)
...
def main(configFilePath, dateId):
    ...
    ExcelTypeId = cfgRoot.attrib['ExcelTypeId']
    ...
    errtxtFilePath = os.path.join(destFolder, 'Error_第三步逻辑校验结果_{0}.xlsx'.format(dateId))

    dbCheckResult = runDBDataValidation(type, poolType, ExcelTypeId, ImportTime, assetType, TrustId)

    if len(dbCheckResult) > 0:
        wb = Workbook()  # 新建工作簿
        ws1 = wb.active
        wb.save(errtxtFilePath)
        writeLog("第三步入库逻辑校验完成，错误详情请查看:{0}".format(errtxtFilePath))
