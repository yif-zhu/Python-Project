        ...
        for i in data.index:
            ...
            if DatabaseItem == o:
                try:
                    ...
                    PaymentPeriodID = data.loc[i][0].astype(int)
                    BucketSequenceNo = float(data.loc[i][3])
                    BucketSequenceNolist.append(BucketSequenceNo)
                    AmountPercentage = float(data.loc[i][6])
                    CountPercentage = float(data.loc[i][8])
                    Amount = float(data.loc[i][5])
                    Count = float(data.loc[i][7])
...
def checkFileFormat(wb, cfgItems,sourceFilePath, paymentPeriodID):
    ...
    if ckResultLen > 0:
        ...
        i = 1
        logSheet["A{0}".format(i)] = 'Sheet1表错误如下：'
        while i <= ckResultLen:
            logSheet["A{0}".format(i+1)] = ckResult[i - 1]
            i += 1
    if ErrorMsgLen > 0:
        if ckResultLen > 0:
            logSheet = wb["格式检查"]
